...
public final class HeroCatalog {
	...
	static {
		...
		add(melee("axe", "Axe", HeroAttribute.STRENGTH, 36, 7,
				abil("berserkers_call", "Berserker's Call", 14 * 20, EffectType.AOE_STUN, 6, 4, 40),
				abil("battle_hunger", "Battle Hunger", 10 * 20, EffectType.TARGET_NUKE, 8, 12, 60),
				abil("counter_helix", "Counter Helix", 3 * 20, EffectType.AOE_DAMAGE, 7, 3, 0),
				abil("culling_blade", "Culling Blade", 50 * 20, EffectType.TARGET_NUKE, 22, 8, 0)));

		add(melee("pudge", "Pudge", HeroAttribute.STRENGTH, 40, 6,
				abil("meat_hook", "Meat Hook", 12 * 20, EffectType.LINE_DAMAGE, 12, 14, 0),
				abil("rot", "Rot", 0, EffectType.AOE_DOT, 3, 4, 80),
				abil("flesh_heap", "Flesh Heap", 20 * 20, EffectType.BUFF_SELF, 0, 0, 200),
				abil("dismember", "Dismember", 25 * 20, EffectType.TARGET_NUKE, 16, 5, 40)));

		add(melee("sven", "Sven", HeroAttribute.STRENGTH, 38, 8,
				abil("storm_hammer", "Storm Hammer", 11 * 20, EffectType.LINE_DAMAGE, 10, 12, 30),
				abil("great_cleave", "Great Cleave", 8 * 20, EffectType.AOE_DAMAGE, 9, 4, 0),
				abil("warcry", "Warcry", 18 * 20, EffectType.BUFF_SELF, 0, 0, 160),
				abil("gods_strength", "God's Strength", 45 * 20, EffectType.BUFF_SELF, 0, 0, 300)));
		...
		add(melee("tiny", "Tiny", HeroAttribute.STRENGTH, 42, 9,
				...
				abil("tree_grab", "Tree Grab", 12 * 20, EffectType.BUFF_SELF, 0, 2, 0),
				abil("grow", "Grow", 0, EffectType.PASSIVE, 0, 0, 0)));

		add(melee("legion", "Legion Commander", HeroAttribute.STRENGTH, 37, 7,
				abil("overwhelming_odds", "Overwhelming Odds", 10 * 20, EffectType.AOE_DAMAGE, 9, 5, 0),
				abil("press_the_attack", "Press The Attack", 12 * 20, EffectType.HEAL, 10, 0, 100),
				abil("moment_of_courage", "Moment of Courage", 6 * 20, EffectType.BUFF_SELF, 0, 0, 60),
				abil("duel", "Duel", 35 * 20, EffectType.TARGET_NUKE, 18, 6, 50)));

		// —— Agility ——
		add(melee("jugg", "Juggernaut", HeroAttribute.AGILITY, 32, 8,
				abil("blade_fury", "Blade Fury", 16 * 20, EffectType.AOE_DOT, 5, 4, 80),
				abil("healing_ward", "Healing Ward", 30 * 20, EffectType.HEAL, 12, 0, 0),
				abil("blade_dance", "Blade Dance", 8 * 20, EffectType.BUFF_SELF, 0, 0, 100),
				abil("omnislash", "Omnislash", 50 * 20, EffectType.TARGET_NUKE, 20, 10, 0)));

		add(melee("pa", "Phantom Assassin", HeroAttribute.AGILITY, 30, 9,
				abil("stifling_dagger", "Stifling Dagger", 6 * 20, EffectType.LINE_DAMAGE, 9, 16, 0),
				abil("phantom_strike", "Phantom Strike", 8 * 20, EffectType.BLINK, 0, 10, 40),
				abil("blur", "Blur", 20 * 20, EffectType.BUFF_SELF, 0, 0, 160),
				abil("coup_de_grace", "Coup de Grace", 0, EffectType.BUFF_SELF, 0, 0, 400)));

		add(melee("am", "Anti-Mage", HeroAttribute.AGILITY, 31, 8,
				abil("mana_break", "Mana Break", 5 * 20, EffectType.TARGET_NUKE, 7, 6, 0),
				abil("blink", "Blink", 7 * 20, EffectType.BLINK, 0, 14, 0),
				abil("counterspell", "Counterspell", 12 * 20, EffectType.BUFF_SELF, 0, 0, 80),
				abil("mana_void", "Mana Void", 40 * 20, EffectType.AOE_DAMAGE, 16, 6, 0)));

		add(ranged("drow", "Drow Ranger", HeroAttribute.AGILITY, 30, 8,
				abil("frost_arrows", "Frost Arrows", 4 * 20, EffectType.LINE_DAMAGE, 6, 18, 40),
				abil("gust", "Gust", 12 * 20, EffectType.AOE_STUN, 4, 5, 30),
				abil("multishot", "Multishot", 14 * 20, EffectType.LINE_DAMAGE, 10, 16, 0),
				abil("marksmanship", "Marksmanship", 0, EffectType.BUFF_SELF, 0, 0, 600)));

		add(ranged("sniper", "Sniper", HeroAttribute.AGILITY, 28, 9,
				abil("shrapnel", "Shrapnel", 12 * 20, EffectType.AOE_DOT, 4, 6, 100),
				abil("headshot", "Headshot", 6 * 20, EffectType.TARGET_NUKE, 10, 20, 0),
				abil("take_aim", "Take Aim", 10 * 20, EffectType.BUFF_SELF, 0, 0, 120),
				abil("assassinate", "Assassinate", 25 * 20, EffectType.LINE_DAMAGE, 22, 28, 0)));
	...
	private static AbilityDef abil(String id, String name, int cd, EffectType type, float power, float radius, int duration) {
		return new AbilityDef(id, name, Math.max(cd, 20), type, power, radius, duration);
	}
